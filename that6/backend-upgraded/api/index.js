...
const FROM_EMAIL = process.env.FROM_EMAIL || "Nuvessia <noreply@nuvessia.pro.et>";
...
const PROVIDERS = {
  ...
  cerebras:   { baseUrl: "https://api.cerebras.ai/v1",           apiKey: () => getRotatingKey("CEREBRAS_API_KEY"),   models: { default: "llama-3.3-70b",                    vision: "llama-3.3-70b" } },
  openrouter: { baseUrl: "https://openrouter.ai/api/v1",         apiKey: () => getRotatingKey("OPENROUTER_API_KEY"), models: { default: "mistralai/mistral-7b-instruct:free", vision: "mistralai/mistral-7b-instruct:free" } },
};
...
app.use((_req, res, next) => {
  ...
  res.setHeader("Content-Security-Policy",
    ...
    "connect-src 'self' https://*.supabase.co https://api.groq.com https://api.cerebras.ai https://openrouter.ai;"
  );
  next();
});
...
// Known devices store (in production use Supabase table)
const knownDevices = new Map(); // userId:deviceId -> true

async function checkAndNotifyNewDevice(req, user) {
  const deviceId = getDeviceId(req);
  const key = `${user.id}:${deviceId}`;
  if (knownDevices.has(key)) return; // known device, skip
  knownDevices.set(key, true);
...
// Step 2: Verify OTP → sign in or sign up via Supabase
app.post("/api/auth/otp/verify", authLimit, async (req, res) => {
  try {
    ...
    // Sign in with Supabase magic link approach — use OTP signin
    const { data, error } = await supabase.auth.signInWithOtp({
      email,
      options: { shouldCreateUser: true },
    });

    // Since we verified OTP ourselves, now get/create user
    // Try signing in, if fails try signing up
    let session = null;
    const signInResult = await supabase.auth.admin.listUsers();
    const existingUser = signInResult.data?.users?.find(u => u.email?.toLowerCase() === email.toLowerCase());
    ...
    if (existingUser) {
      ...
      // Exchange the token from the link
      const token = linkData.properties?.hashed_token;
      if (token) {
        const { data: sessionData } = await supabase.auth.verifyOtp({ token_hash: token, type: "magiclink" });
    ...
    res.json({ ok: true, session, message: "Signed in successfully" });
  } catch (e) {
