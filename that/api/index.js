...
app.get("/api/lookup/whois", async (req, res) => {
  ...
  const result = await new Promise((resolve) => {
    let done = false;
    const timer = setTimeout(() => { if (!done) { done = true; resolve({ error: "Timed out" }); } }, 8000);
    try {
      whois.lookup(q, { timeout: 7000 }, (err, data) => {
        if (done) return; done = true; clearTimeout(timer);
        resolve(err ? { error: err.message } : { raw: String(data || "") });
      });
    } catch (e) { if (!done) { done = true; clearTimeout(timer); resolve({ error: e.message }); } }
  });
...
app.post("/api/chat", rateLimit, upload.any(), async (req, res) => {
  try {
    ...
    if (imageFiles.length > 0) {
      finalMessages = [...prevMsgs, { role: "user", content: [{ type: "text", text: userText }, ...imageFiles.map((f) => ({ type: "image_url", image_url: { url: `data:${f.mimetype};base64,${f.buffer.toString("base64")}` } }))] }];
    } else {
      finalMessages = [...prevMsgs, { role: "user", content: userText }];
    }
